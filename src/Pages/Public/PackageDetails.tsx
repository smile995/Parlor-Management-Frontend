...
const PackageDetails = () => {
  ...
  if (isError) return <div className="py-20 text-center text-red-500">Failed to load package details.</div>;
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      >
        ...
        <div className="absolute bottom-6 left-6 text-white">
          ...
          <p className="mt-2 max-w-lg text-gray-200">{pkg.description}</p>
          {pkg.status && (
            <span
      ...
      <div className="max-w-5xl mx-auto p-6">
        ...
        <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <InfoCard icon={<FaClock />} label="Duration" value={pkg.duration} />
          <InfoCard
            icon={<FaTag />}
            label="Total Price"
            value={
          ...
          />
          <InfoCard icon={<FaStar />} label="Rating" value={pkg.rating ?? "N/A"} />
          <InfoCard
            icon={<FaUsers />}
            label="Bookings"
            value={pkg.totalBookings ?? 0}
          />
        </div>
        ...
        {pkg.preparation && (
          <Section title="Preparation Tips">
            <p className="text-gray-700">{pkg.preparation}</p>
          </Section>
        )}

        {/* Services */}
        {pkg.servicesNames?.length > 0 && (
          <Section title="Included Services">
            <ul className="list-disc ml-6 space-y-2 text-gray-700">
              {pkg.servicesNames.map((service: string, i: number) => (
                <li key={i}>{service}</li>
              ))}
            </ul>
          </Section>
        ...
        {/* Staff */}
        {pkg.availableStaff?.length > 0 && (
          <Section title="Available Staff">
            <div className="flex flex-wrap gap-3">
              {pkg.availableStaff.map((staff: string, i: number) => (
                <div
                  key={i}
      ...
      <div className="fixed bottom-0 left-0 w-full bg-white border-t p-4 flex justify-between items-center shadow-lg">
        <div>
          ...
          <p className="text-xl font-bold text-pink-600">৳{pkg.totalPrice}</p>
        </div>
